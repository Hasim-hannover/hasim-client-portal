...
export default async function AdminPage({
  ...
  const [profilesResult, projectsResult, messagesResult, filesResult] = await Promise.all([
    ...
    supabase
      .from("projects")
      .select("id, client_id, name, status, phase, phase_note, created_at, updated_at")
    ...
    supabase
      .from("project_files")
      .select("id, project_id, file_name, size_bytes, created_at")
      .order("created_at", { ascending: false })
      .limit(12),
  ]);
  ...
  return (
    <div className="portal-shell admin-shell">
      ...
      <main className="main admin-main" id="overview">
        ...
        <p className="lead">
          Kunden einladen, Projekte anlegen, Projektphasen aktualisieren, Dateien bereitstellen und Nachrichten senden.
        </p>
        ...
        <section className="admin-grid" id="clients">
          <article className="admin-panel">
            <div className="section-heading compact-heading">
              <div><div className="eyebrow">Kundenverwaltung</div><h2>Neuen Kunden einladen</h2></div>
            </div>
            ...
            <p className="muted admin-hint">Für echte Einladungen muss der serverseitige Supabase-Admin-Key in Vercel hinterlegt sein.</p>
          </article>
          ...
          <article className="admin-panel">
            ...
            {clients.length === 0 ? (
              <div className="empty-state">Noch keine Kundenkonten angelegt.</div>
            ) : (
              <div className="admin-list">
                {clients.map((client) => (
                  <div className="admin-list-row" key={client.id}>
                    <div><strong>{client.full_name || "Ohne Namen"}</strong><span>{client.email || "Keine E-Mail"}</span></div>
                    <span className="badge">{projects.filter((project) => project.client_id === client.id).length} Projekte</span>
                  </div>
                ))}
              </div>
            )}
        ...
        <section className="admin-panel" id="projects">
          <div className="section-heading">
            <div><div className="eyebrow">Projektsteuerung</div><h2>Projekte & aktuelle Phase</h2></div>
            <span className="badge">{projects.length}</span>
          </div>

          {clients.length ? (
            <form action={createProject} className="admin-create-project-form">
              <label>Kunde<select name="clientId" required>{clients.map((client) => <option key={client.id} value={client.id}>{client.full_name || client.email}</option>)}</select></label>
              <label>Projektname<input name="name" required placeholder="Website Relaunch" /></label>
                  ...
                  <article className="admin-project-card" key={project.id}>
                    <div className="admin-project-head">
                      <div>
                        ...
                        <span>{client?.full_name || client?.email || "Kunde"} · aktualisiert {formatDate(project.updated_at)}</span>
                      </div>
                    ...
                    <form action={updateProjectPhase} className="admin-phase-form">
                      ...
                      <label>Phase<select name="phase" defaultValue={project.phase}>{phases.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
                      <label className="admin-phase-note">Hinweis für den Kunden<textarea name="phaseNote" rows={3} maxLength={1200} defaultValue={project.phase_note ?? ""} placeholder="Zum Beispiel: Inhalte sind vollständig, Umsetzung startet diese Woche." /></label>
        ...
        <section className="admin-grid">
          <article className="admin-panel">
            ...
            {messages.length === 0 ? <div className="empty-state">Keine Nachrichten.</div> : (
              <div className="admin-list">{messages.map((message) => {
                const project = projectById.get(message.project_id);
                const sender = profileById.get(message.sender_id);
                return <div className="admin-list-row admin-message-row" key={message.id}><div><strong>{sender?.full_name || sender?.email || "Unbekannt"}</strong><span>{project?.name || "Projekt"} · {formatDate(message.created_at)}</span><p>{message.body}</p></div></div>;
              })}</div>
            )}
          </article>
          ...
          <article className="admin-panel">
            ...
            {files.length === 0 ? <div className="empty-state">Keine Dateien.</div> : (
              <div className="admin-list">{files.map((file) => <div className="admin-list-row" key={file.id}><div><strong>{file.file_name}</strong><span>{projectById.get(file.project_id)?.name || "Projekt"} · {formatDate(file.created_at)}</span></div></div>)}</div>
            )}
          </article>
        </section>
      </main>
