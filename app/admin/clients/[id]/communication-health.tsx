  ...
  return (
    <section className="admin-panel communication-health" id="communication-health" aria-labelledby="communication-health-title">
      <div className="section-heading">
        ...
        <span className={failed ? "delivery-summary has-errors" : "delivery-summary is-ok"}>{failed ? <TriangleAlert size={14} aria-hidden="true" /> : <MailCheck size={14} aria-hidden="true" />}{failed ? `${failed} auffällig` : "Versand stabil"}</span>
      </div>
        ...
        <div className="delivery-list">
          {rows.map((row) => (
            <div className={`delivery-row ${row.ok ? "is-ok" : "is-error"}`} key={row.id}>
              <span className="delivery-icon">{row.ok ? <CheckCircle2 size={16} aria-hidden="true" /> : <TriangleAlert size={16} aria-hidden="true" />}</span>
              <div>
                <strong>{labels[row.kind] ?? row.kind}</strong>
                <span>{formatDate(row.created_at)} · Brevo {row.provider_status || "—"}</span>
