...
export function UploadPanel({
  ...
  const initialRequest = requests.find((request) => request.id === initialRequestId && request.status !== "done");
  ...
  const projectRequests = requests.filter(
    (request) => request.projectId === projectId && request.status !== "done",
  );
  ...
  function handleProjectChange(nextProjectId: string) {
    ...
    if (!requests.some((request) => request.id === requestId && request.projectId === nextProjectId)) {
      setRequestId("");
    }
  }
    ...
    <section className="upload-panel" id="upload" aria-labelledby="upload-title">
      <div className="section-heading compact-heading">
        <div>
          ...
          <h2 id="upload-title">Dateien hochladen</h2>
      ...
      <form className="upload-form upload-form-expanded" onSubmit={handleSubmit} aria-busy={uploading}>
        ...
        <select id="upload-request" value={requestId} onChange={(event) => handleRequestChange(event.target.value)} disabled={uploading}>
          ...
          {projectRequests.map((request) => (
            <option key={request.id} value={request.id}>
              {request.status === "submitted" ? "Bereits eingereicht: " : "Benötigt: "}{request.title}
            </option>
          ))}
        </select>
        ...
        <label className="upload-files-field" htmlFor="upload-files">Dateien</label>
        <input
          id="upload-files"
        ...
        {files.length ? (
          <ul className="selected-files" aria-label="Ausgewählte Dateien">
            {files.map((file) => <li key={`${file.name}-${file.size}`}>{file.name}</li>)}
          </ul>
        ) : null}

        <label className="upload-note-field" htmlFor="upload-note">Notiz zum Upload <span className="optional-label">optional</span></label>
        <textarea
          id="upload-note"
          className="upload-note-field"
          value={note}
