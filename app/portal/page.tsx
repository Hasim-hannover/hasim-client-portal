...
export default async function PortalPage() {
  ...
  const [projectsResult, filesResult, messagesResult] = await Promise.all([
    ...
    supabase.from("project_files").select("id, project_id, upload_id, category, note, file_name, storage_path, mime_type, size_bytes, created_at").order("created_at", { ascending: false }).limit(60),
  ...
  const fileList = await Promise.all((filesResult.data ?? []).map(async (file) => {
    const { data } = await supabase.storage.from("project-files").createSignedUrl(file.storage_path, 60 * 10);
    return { ...file, signedUrl: data?.signedUrl ?? null, projectName: projectNames.get(file.project_id) ?? "Projekt" };
  }));
      ...
      <main className="main" id="dashboard">
        <section className="portal-hero-grid">
          <div className="portal-hero-copy">
            ...
            <p className="lead">Lade Projektmaterial sicher hoch, verfolge die aktuelle Projektphase und tausche Nachrichten direkt im Projekt aus.</p>
          ...
          <aside className="help-card" aria-label="So funktioniert das Kundenportal">
            <div className="help-card-title"><HelpCircle size={20} /><div><div className="eyebrow">Kurz erklärt</div><h2>So funktioniert der Dateiaustausch</h2></div></div>
            <p>Lade hier alle Dateien hoch, die ich für dein Projekt benötige – zum Beispiel Bilder, Logos, Dokumente oder Videos.</p>
            <div className="help-steps">
              ...
              <div><LayoutDashboard size={17} /><span>Projektstatus und Nachrichten bleiben zentral im Portal.</span></div>
            </div>
          </aside>
        </section>

        {loadError ? <div className="portal-warning">Ein Teil der Projektdaten konnte nicht geladen werden. Bitte Seite neu laden.</div> : null}

        <section className="project-status-section" id="projektstatus">
          <div className="section-heading"><div><div className="eyebrow">Projektfortschritt</div><h2>Aktuelle Phase</h2></div></div>
              ...
              {projectList.map((project) => {
                ...
                return (
                  <article className="project-status-card" key={project.id}>
                    ...
                    <div className="phase-track" aria-label={`Aktuelle Projektphase: ${phaseLabels[project.phase] ?? project.phase}`}>
                      {phaseOrder.map((phase, index) => <span key={phase} className={index <= currentIndex ? "phase-step active" : "phase-step"} title={phaseLabels[phase]} />)}
                    </div>
        ...
        <div id="upload"><UploadPanel projects={compactProjects} /></div>

        <section className="files-section" id="dateien">
          <div className="section-heading"><div><div className="eyebrow">Projektdateien</div><h2>Zuletzt hochgeladen</h2></div><span className="badge">{fileList.length} Dateien</span></div>
          {uploadGroups.length === 0 ? <div className="empty-state">Noch keine Dateien vorhanden.</div> : (
            <div className="upload-history">{uploadGroups.map((group) => (
              <article className="upload-history-group" key={group.id}>
                ...
                <div className="file-list">{group.files.map((file) => <div className="file-row" key={file.id}><div><strong>{file.file_name}</strong><span>{formatBytes(file.size_bytes)}</span></div>{file.signedUrl ? <a className="secondary-button" href={file.signedUrl} target="_blank" rel="noreferrer" download>Herunterladen</a> : <span className="muted">Nicht verfügbar</span>}</div>)}</div>
              </article>
            ))}</div>
          )}
        </section>
        ...
        <section className="messages-history" aria-label="Letzte Projektnachrichten">
          <div className="section-heading"><div><div className="eyebrow">Kommunikation</div><h2>Letzte Nachrichten</h2></div><span className="badge">{messages.length}</span></div>
        ...
        <section className="status"><div><strong>Privater Projektspeicher</strong><span>Dateien liegen in einem privaten Supabase-Bucket und werden nur zeitlich begrenzt freigegeben.</span></div><div className="badge"><LayoutDashboard size={14} style={{ verticalAlign: "-2px", marginRight: 6 }} />MVP</div></section>
      </main>
    </div>
  );
}
